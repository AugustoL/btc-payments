                ...
                if (address){
                    ...
                    db.poolAddresses.find({},{},function(err,allAddresses){
                        if (!err){
                            ...
                            logger.success("Derivating form master key on m/"+deriveIndex+"'");
    ...
    */
    function checkAddressWaiting(addressW,callback){
        ...
        async.waterfall([
            ...
            //Get PaymentWaiting
            function(callback){
                logger.success('Looking for tx waiting id: '+addressW.paymentWaiting);
            ...
            function(utxos,paymentW,callback){
                logger.success('Checking '+JSON.parse(utxos).data.unspent.length+' utxos..');
                    ...
                    if ((config.warningTimeout > 0) && paymentW && (!paymentW.warned) && (unspent.length == 0) && ( (((new Date(paymentW.dateBegin.getTime() + config.warningTimeout*60000))-new Date())/60000) < 0)){                      
                        paymentW.warn(function(err,done){
                            if (err){
                                ...
                                logger.success('PaymentW warning..');
                                callback(null,true,0);
                            }
                        });
