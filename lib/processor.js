...
module.exports = function(db,logger,config) {
    ...
    module.update = function(callback){
        ...
        //All addresses waiting checked   
            ...
            if (err){
                ...
                logger.success("Finish updating addresses!");
                if (callback)
                    callback(null);
            }
        });
    ...
    module.getFreeAddress = function(newPaymentWaiting,callback){
        var freeAddress = "empty";
        var knownBytes = bitcore.crypto.Hash.sha256(new Buffer(config.seedBytes));
        var masterKey = new bitcore.HDPrivateKey.fromSeed(knownBytes,config.network);
        async.waterfall([
            function(callback){
                ...
                if (address){
                    ...
                    address.startWaiting(newPaymentWaiting.id,function(err){
                        if (err)
                            ...
                            logger.success("New address for payment obtained: "+freeAddress);
                    ...
                    db.poolAddresses.find({},{},function(err,allAddresses){
                        if (!err){
                            ...
                            var derived = masterKey.derive('m/'+deriveIndex+"'");
                            var derivedPriv = new bitcore.PrivateKey(derived.privateKey, config.network)
                            ...
                            newAddress.create(deriveIndex,derivedPriv,new bitcore.PublicKey.fromPrivateKey(derivedPriv),new bitcore.Address.fromPublicKey(new bitcore.PublicKey.fromPrivateKey(derivedPriv),config.network).toString(),true,newPaymentWaiting,function(err){
                                if (err){
                                    ...
                                    logger.success("New address for payment generated: "+newAddress.string);
    ...
    function checkAddressWaiting(addressW,callback){
        ...
        async.waterfall([
            ...
            function(utxos,paymentW,callback){
                ...
                checkUtxos(utxos,addressW,function(err,txs,amountW,unusedBalance,unspent){
                    if (err) {
                        callback(err);
                    } else {
                        callback(null,paymentW,txs,amountW,unusedBalance,unspent);
                    }
                });
            },

            //All utxos in AddressWaiting checked, now parse and sendTX if have to
            function(paymentW,txs,amountW,unusedBalance,unspent,callback){ 
                if ((amountW >= paymentW.quantity)&&(unusedBalance >= config.limitBalance)&&(unspent.length > 0)){
                    ...
                    var privKeys = [addressW.privKey.toString()];
                    //Sending TX
                    sendTX(unspent,to,privKeys,function(err,done){
                        if (err) {
                            callback(err,null,null);
                        } else if (done) {
                            paymentW.complete(addressW,txs,function(err,done){
                                if (err){
                                    callback(err,null,null);
                                } else {
                                    logger.success("Tx parsed for address: "+addressW.string+" using txs: "+txs.toString());
                    ...
                    logger.success('Parsing tx to done with txs: '+txs.toString());  
                    paymentW.complete(addressW,txs,function(err,done){
                        if (err){
                            callback(err);
                        } else {
                    ...
                    if (paymentW && (unspent.length == 0) && ( (((new Date(paymentW.dateBegin.getTime() + config.paymentTimeout*60000))-new Date())/60000) < 0)){
                        paymentW.remove();
                        logger.success('PaymentW timeout, removed.');
                        callback(null,true,0);
                    } else {
                        logger.success('Nothing to parse on address..');
            ...
            function(finishWaiting,unusedBalance,callback){
                if (finishWaiting){
                    logger.success('Address finishing waiting..');
    ...
    function checkUtxos(utxos,addressW,callback){
        ...
        async.forEachLimit(JSON.parse(utxos).data.unspent,1,function(utxo, callback){
                ...
                function(callback){
                    db.paymentsDone.find({ txs: { $in : [utxo.tx]} }, {},function(err,paymentDone){
                        if (err){
                            ...
                            //logger.log('Adding unspent..');
                            callback(null,true,utxo);
                        } else {
                            //logger.log('Adding unspent, amountW and txs to be parsed to Done..');
            ...
            if (err)
                ...
                callback(null,txs,amountW,unusedBalance,unspent);
        });
    }
