...
module.exports = function(db,logger,onComplete,onCancel,onWarning) {

	function completeTX(payment,callback) {
		if (onComplete['*']){
			onComplete['*'](payment,function(err,done){
				callback(err,done);
			})
		} else if (onComplete[payment.opName]){
			onComplete[payment.opName](payment,function(err,done){
				callback(err,done);
			})
		} else {
	...
	function cancelTX(payment,callback) {
		if (onCancel['*']){
			onCancel['*'](payment,function(err,done){
				callback(err,done);
			})
		} else if (onCancel[payment.opName]){
			onCancel[payment.opName](payment,function(err,done){
				callback(err,done);
			})
		} else {
	...
	function warnTX(payment,callback) {
		if (onWarning['*']){
			onWarning['*'](payment,function(err,done){
				callback(err,done);
			})
		} else if (onWarning[payment.opName]){
			onWarning[payment.opName](payment,function(err,done){
				callback(err,done);
			})
		} else {
	...
	var paymentWaiting = new Schema({
		...
		warned : { type : Boolean, required : true },
	...
	paymentWaiting.methods.complete = function(addressW,fromAddress,txs,callback) {
		...
		async.waterfall([
			...
			function(callback){
				completeTX(self,function(err,done){
		            if (err)
		                callback(err);
		            else
		                callback(null,done);
	...
	paymentWaiting.methods.cancel = function(execFunc,callback) {
		...
		async.waterfall([
		    ...
		    function(callback){
		    	if (execFunc){
		    		cancelTX(self,function(err,done){
			            if (err)
			                callback(err);
			            else
			                callback(null,done);
