...
module.exports = function(db,logger,onComplete,onCancel) {

	function completeTX(opName,otherData,callback) {
		if (onComplete['*']){
			onComplete['*'](otherData,function(err,done){
				callback(err,done);
			})
		} else if (onComplete[opName]){
			onComplete[opName](otherData,function(err,done){
				callback(err,done);
			})
		} else {
			callback('Operation with that name isnt defined on onComplete.');
		}
	}

	function cancelTX(opName,otherData,callback) {
		if (onCancel['*']){
			onCancel['*'](otherData,function(err,done){
				callback(err,done);
			})
		} else if (onCancel[opName]){
			onCancel[opName](otherData,function(err,done){
				callback(err,done);
			})
		} else {
			callback('Operation with that name isnt defined on onComplete.');
	...
	paymentWaiting.methods.complete = function(addressW,txs,callback) {
		...
		var self = this;
		var opName = self.opName;
		var otherData = self.otherData;
		async.waterfall([
			function(callback){
				newPaymentDone = new db.paymentsDone();
				newPaymentDone.create(txs,self.dateBegin,opName,addressW.string,self.toAddress,self.quantity,otherData);
			...
			function(callback){
				completeTX(opName,otherData,function(err,done){
		            if (err)
		                callback(err);
		            else
		                callback(null,done);
	...
	paymentWaiting.methods.cancel = function(execFunc,callback) {
		...
		async.waterfall([
		    ...
		    function(callback){
		    	if (execFunc){
		    		cancelTX(opName,otherData,function(err,done){
			            if (err)
			                callback(err);
			            else
			                callback(null,done);
